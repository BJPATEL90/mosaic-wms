...
/* ═══════════════════════════════════════════════════════
   ...
   Handles: Upload, Parse, Current Inventory view,
            Gatepass upload, Templates, Side stats
   Depends on: utils.js, app.js (state + index maps)
═══════════════════════════════════════════════════════ */
...
function parseInvFile(ev){
  ...
  parseExcel(f, (err, rows) => {
    ...
    if(!cm.skuCode || !cm.qty){
      msg.innerText = '❌ SKU Code or Qty column not found';
      toast('Required columns missing', 'err'); return;
    }
    ...
    const total = skipped.blankSku + skipped.zeroQty + skipped.other;
    let detail = '';
    if(skipped.blankSku > 0) detail += skipped.blankSku + ' blank SKU, ';
    if(skipped.zeroQty  > 0) detail += skipped.zeroQty  + ' zero/negative qty (normal for 0-stock rows), ';
    if(skipped.other    > 0) detail += skipped.other     + ' other, ';
    detail = detail.replace(/, $/, '');

    msg.innerText =
      `✓ ${inventory.length} rows loaded from ${f.name}.` +
      (total ? ` ⚠ ${total} skipped (${detail}).` : '');
    ...
    $('inv-chips').innerHTML =
      ...
      (total ? `<span class="chip chip-warn" title="${detail}">⚠ ${total} skipped</span>` : '');
    ...
    if(gasUrl()){
      syncSheet('uploadInventoryDump', inventory).then(r => {
        ...
        else if(r && r.message && !r.message.includes('GAS_URL'))
          toast('⚠ Sheet sync: ' + r.message, 'warn');
      });
    }
  });
  ev.target.value = '';
...
function updateInvNav(){
  $('nav-inv-ct').innerText = inventory.length || '';
}

/* ══════════════════════════════════════════════
   INVENTORY DUMP TEMPLATE
══════════════════════════════════════════════ */
function dlInvTpl(){
  ...
  XLSX.utils.book_append_sheet(wb, ws, 'Inventory Dump');
  XLSX.writeFile(wb, 'Inventory_Dump_Template.xlsx');
}

/* ══════════════════════════════════════════════
   CURRENT INVENTORY SECTION
   Filterable, paginated, FEFO-sorted table
══════════════════════════════════════════════ */
...
function applyInvFilters(){
  ...
  invFiltered = inventory.filter(r => {
    const type = str(r.stockType||'GOOD').toUpperCase();
    ...
    if(qType && type !== qType) return false;
    if(qExp){
  ...
  const dmg       = invFiltered.filter(r => {
    const t = str(r.stockType||'GOOD').toUpperCase();
...
function renderInvPage(){
  ...
  if(!slice.length){
    body.innerHTML = '<tr class="er"><td colspan="12">No rows match filters</td></tr>';
    renderPgn(); return;
  }
  ...
  body.innerHTML = slice.map((r, i) => {
    ...
    if(r.qty < 0){
      rowCls = 'row-negative';
      flags.push('<span class="badge b-err" style="font-size:8px">NEGATIVE</span>');
    } else if(days !== null && days <= 0){
      rowCls = 'row-expiry';
      flags.push('<span class="badge b-err" style="font-size:8px">EXPIRED</span>');
    } else if(days !== null && days <= 30){
      rowCls = 'row-expiry';
      flags.push('<span class="badge b-warn" style="font-size:8px">≤30D</span>');
    } else if(days !== null && days <= 90){
      flags.push('<span class="badge b-info" style="font-size:8px">≤90D</span>');
    }
    ...
    return `<tr class="${rowCls}">
      ...
      <td>
        <span class="badge ${r.stockType==='DAMAGE'||r.stockType==='DAMAGED' ? 'b-err' : r.stockType==='BLOCKED' ? 'b-warn' : 'b-ok'}" style="font-size:9px">
          ${esc(r.stockType)}
        </span>
      </td>
      ...
      <td style="text-align:right;font-family:ui-monospace,monospace;font-weight:${gpQ>0?'700':'400'};color:${gpQ>0?'var(--red)':'var(--slate5)'}">
        ${gpQ > 0 ? gpQ.toLocaleString() : '—'}
      </td>
      <td style="text-align:right;font-family:ui-monospace,monospace;font-weight:700;color:${net<0?'var(--red)':'var(--teal)'}">
        ${net.toLocaleString()}
      </td>
      <td>${flags.join(' ')}</td>
    </tr>`;
...
function renderPgn(){
  ...
  p.innerHTML =
    `<button onclick="invPage=1;renderInvPage()" ${invPage===1?'disabled':''}>«</button>
...
function exportInvXLSX(){
  ...
  const rows = inventory.map(r => ({
    ...
    'EAN':            r.ean,
    'Days to Expiry': daysToExp(r.expDate),
  }));
  const ws = XLSX.utils.json_to_sheet(rows);
  ws['!cols'] = [
    {wch:14},{wch:28},{wch:34},{wch:12},{wch:12},{wch:12},
    {wch:8},{wch:10},{wch:10},{wch:8},{wch:12},{wch:14},{wch:12}
  ];
  const wb = XLSX.utils.book_new();
